...
export const StudentLayout: React.FC = () => {
  ...
  const handleLogout = async () => {
    if (window.confirm('Apakah kamu yakin ingin keluar dari akun siswa?')) {
      await logout();
      navigate('/login');
    }
  };
  ...
  const navItems = [
    { to: '/student/dashboard', label: 'Beranda Siswa', icon: Home },
    { to: '/student/scan', label: 'Kode QR Presensi Saya', icon: QrCode },
    { to: '/student/history', label: 'Riwayat Kehadiran', icon: History },
    { to: '/student/profile', label: 'Profil & Status Device', icon: User },
  ];
  ...
  return (
    <div className="h-screen h-[100dvh] w-full bg-slate-50 text-slate-900 flex relative overflow-hidden">
      ...
      <aside className="hidden lg:flex lg:flex-col w-64 h-full bg-white/90 backdrop-blur-xl border-r border-slate-200/90 p-4 shrink-0 shadow-xs z-30 select-none">
        ...
        <div className="flex items-center space-x-3 px-2 py-3 mb-5 border-b border-slate-100">
          ...
          <div>
            <div className="flex items-center space-x-1.5">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 font-bold border border-blue-200 uppercase">
                Siswa
              </span>
            </div>
        ...
        {student && (
          <div className="mb-6 p-3 rounded-2xl bg-slate-50/80 border border-slate-200/80 flex items-center space-x-3 transition-transform hover:scale-[1.01]">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-white font-extrabold text-sm shadow-xs overflow-hidden shrink-0">
              {student.foto_url ? (
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold text-slate-900 truncate">{student.nama}</p>
              <p className="text-[11px] text-slate-500 font-medium">Absen #{student.nomor_absen} • XI PPLG 3</p>
            </div>
          </div>
        )}
          ...
          {navItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                to={item.to}
                className={({ isActive }) =>
                  `flex items-center space-x-3 px-3.5 py-3 rounded-xl text-xs font-semibold transition-all duration-200 ${
                    isActive
        ...
        {/* Quick Tips */}
        <div className="my-4 p-3.5 rounded-2xl bg-blue-50/70 border border-blue-100 text-xs text-blue-800 space-y-1">
          ...
          <p className="text-[11px] text-blue-700 leading-relaxed">
            Absen sebelum {activeSession?.batas_terlambat || '06:45'} WIB • Validasi titik koordinat GPS real-time HP siswa.
          </p>
        </div>
      ...
      {/* Mobile Drawer (Hamburger Slide-Over Sidebar) */}
      {mobileMenuOpen && (
        <div
          className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 lg:hidden transition-opacity duration-200"
      ...
      >
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-200 mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="font-extrabold text-sm text-slate-900 font-heading">PPLG 3 SISWA</p>
              <p className="text-xs text-slate-500 font-medium">SMKN 1 Ciomas</p>
            </div>
          </div>
          <button 
        ...
        {student && (
          <div className="mb-5 p-3 rounded-2xl bg-slate-50 border border-slate-200 flex items-center space-x-3">
            <div className="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center text-white font-bold text-sm overflow-hidden shrink-0">
              {student.foto_url ? (
                <img src={student.foto_url} alt={student.nama} className="w-full h-full object-cover" />
              ) : (
                student.nama.charAt(0)
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold text-slate-900 truncate">{student.nama}</p>
              <p className="text-[11px] text-slate-500">Absen #{student.nomor_absen} • XI PPLG 3</p>
            </div>
          </div>
        )}
      ...
      >
        ...
        {/* Mobile Top Header Bar with Hamburger Button & Bottom Progress Indicator */}
        <header className="lg:hidden sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200 px-4 py-3 flex items-center justify-between shadow-xs relative">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="font-extrabold text-sm text-slate-900 font-heading">XI PPLG 3</span>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs font-semibold text-slate-700 bg-slate-100 px-2.5 py-1 rounded-lg">
              #{student?.nomor_absen || '-'} {student?.nama?.split(' ')[0] || 'Siswa'}
            </span>
          </div>
        ...
        {/* Page Body */}
        <main className="flex-1 p-3.5 sm:p-5 md:p-6 lg:p-8 max-w-5xl w-full mx-auto overflow-x-hidden animate-page-enter pb-20 sm:pb-12">
          <Outlet />
        </main>
